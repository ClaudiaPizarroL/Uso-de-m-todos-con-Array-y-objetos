...
//Clonar el arreglo traumatologia
const copiaTraumatologia = [...traumatologia];

// Agregar las nuevas horas de Traumatología al arreglo existente
for (let i = 0; i < nuevasHorasTraumatologia.length; i++) {
    copiaTraumatologia.push(nuevasHorasTraumatologia[i]);
}

// Clonar el arreglo de Radiología para no modificar el original
const copiaRadiologia = [...radiologia];

// Eliminar el primer elemento del arreglo clonado
copiaRadiologia.shift();

// Eliminar el último elemento del arreglo clonado
copiaRadiologia.pop();


// 1. Imprimir la lista de consultas médicas de Dental
...
// 2. Imprimir un listado total de todos los pacientes que se atendieron
...
// 3. Filtrar los pacientes de ISAPRE en Dental
document.write('<h3>Pacientes de ISAPRE en consultas de Dental:</h3>');
dental.filter(consulta => consulta.prevision === 'ISAPRE').forEach(consulta => {
    document.write(`<p>${consulta.paciente} - ${consulta.prevision}</p>`);
});

// 4. Filtrar los pacientes de FONASA en Traumatología
document.write('<h3>Pacientes de FONASA en consultas de Traumatología:</h3>');
